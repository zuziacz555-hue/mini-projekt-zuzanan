...
Projekt trzyma się następującego podziału katalogów:

```
/                  ← pliki strony (index.html, style.css, script.js)
data/              ← pliki danych JSON
docs/              ← dokumentacja projektu (Markdown)
pliki-do-branchy/  ← materiały pomocnicze dla każdego brancha
```

Każdy plik ma jedno wyraźne zadanie. Nie łączymy logiki strony z dokumentacją, a danych z kodem.

## Podsumowanie

Praca z Git uczy dyscypliny: każda zmiana ma swój commit, każda nowa funkcja ma swój branch, a gotowy kod trafia do `main`. Dzięki temu historia projektu jest przejrzysta i można wrócić do dowolnego punktu w historii.
